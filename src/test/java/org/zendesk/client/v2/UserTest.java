...
/**
 * An initial attempt at a unit test that uses wiremock to test the client without requiring a
 * running zendesk client
 *
...
public class UserTest {
  ...
  public void mergeUsers() throws JsonProcessingException {
    ...
    String expectedJsonPayload =
        objectMapper.writeValueAsString(Collections.singletonMap("user", userObjectInRequest));
    ...
    String expectedJsonResponse =
        objectMapper.writeValueAsString(Collections.singletonMap("user", userObjectInResponse));

    zendeskApiMock.stubFor(
        put(urlPathEqualTo("/api/v2/users/1234/merge.json"))
    ...
    zendeskApiMock.verify(
        putRequestedFor(urlEqualTo("/api/v2/users/1234/merge.json"))
            .withHeader("Content-Type", equalTo(APPLICATION_JSON + "; charset=UTF-8"))
            .withRequestBody(equalToJson(expectedJsonPayload)));

    assertThat(remainingUser)
        .as("result")
        .isNotNull()
  ...
  @Test
  public void getUsersByExternalIds_canBeCalledWithEitherLongsOrStrings()
    ...
    zendeskApiMock.stubFor(
        get(urlPathEqualTo("/api/v2/users/show_many.json"))
    ...
    zendeskApiMock.verify(
        getRequestedFor(urlPathEqualTo("/api/v2/users/show_many.json"))
            .withQueryParam("external_ids", equalTo("123,456,789")));

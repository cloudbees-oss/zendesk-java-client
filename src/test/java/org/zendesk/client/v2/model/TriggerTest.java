...
public class TriggerTest {
  ...
  public void testTriggerWithComplexActionValues() throws Exception {
    String json =
        Utils.resourceToString(
            getClass().getName().replace('.', '/') + "/triggerWithComplexActions.json");
    ...
    assertArrayEquals(
        new Object[] {
          "1234567890",
          Arrays.asList(Arrays.asList("key1", "value1"), Arrays.asList("key2", "value2"))
        },
  ...
  public void testTriggerWithMultilineActionValues() throws Exception {
    String json =
        Utils.resourceToString(
            getClass().getName().replace('.', '/') + "/triggerWithMultilineAction.json");
    Trigger trigger = parseJson(json.getBytes());
    assertNotNull(trigger);
    assertEquals("Test Trigger", trigger.getTitle());
